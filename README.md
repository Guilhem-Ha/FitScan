...
├── plugins/
│   └── withOkHttpConnectTimeout.js # Native fix for networks with broken IPv6
└── data/
...
Some WiFi networks hand out IPv6 addresses but never route IPv6 traffic.
Android lists IPv6 addresses first, and React Native's OkHttp client has no
connect timeout, so every request hangs on a dead route and never falls back to
IPv4. `plugins/withOkHttpConnectTimeout.js` installs an OkHttp factory in
`MainApplication` with a 5 s connect timeout and a DNS lookup that interleaves
IPv4 and IPv6 addresses, IPv4 first. The timeout alone is not enough:
`generativelanguage.googleapis.com` has six or more AAAA records, and trying
each for 5 s outlasts the app's 30 s request timeout.

It is a native change: `android/` is generated by `expo prebuild`, so edit the
plugin rather than the generated file, and ship it with a new EAS build.
