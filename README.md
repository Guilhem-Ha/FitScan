...
Android tries the IPv6 address first, and React Native's OkHttp client has no
connect timeout, so every request hangs on the dead route and never falls back
to IPv4. `plugins/withOkHttpConnectTimeout.js` installs an OkHttp factory in
`MainApplication` with a 5 s connect timeout, after which OkHttp moves on to the
next (IPv4) address.
